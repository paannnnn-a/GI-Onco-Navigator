...
来源带有版本、日期、癌种、受众、版权、审核状态、替代关系与哈希。切片保留页码或时间戳、章节路径、提取方法和内容哈希。中文全文检索增加 CJK 双字索引；排序优先考虑证据类型。
...
生产部署还需要 HTTPS、密钥托管、强身份认证、最小权限、备份恢复、日志脱敏、漏洞扫描、隐私影响评估和适用地区的医疗/数据合规审查。

## 7. 评测

AI Benchmark 使用完全虚构病例，分别衡量旅程状态、安全分类、检索召回、引用正确性、证据支持率、拒答准确性与危险建议率。临床内容评测集在公开前需要独立专家审核，且不能包含真实患者身份信息。
